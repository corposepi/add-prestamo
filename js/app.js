...
const App = {
        ...
        switch (tipoPago) {
            ...
            case 'monto_libre': {
                ...
                if (montoLibre > 0) {
                    ...
                    capitalPagar = 0; // El sobrante NO va a capital
                    total = montoLibre;
                    nuevoSaldo = prestamo.saldoCapital; // Capital no cambia
    ...
    async guardarPago() {
        ...
        confirmMsg += `<strong>Total pagado:</strong> ${Utils.formatMoney(total)}<br>`;
        if (saldoFavorNuevo > 0) {
            confirmMsg += `<strong>Saldo a favor (prox. cuota):</strong> ${Utils.formatMoney(saldoFavorNuevo)}<br>`;
        }
        ...
        const datos = { montoAbono: capitalPagar, montoPago: total };
